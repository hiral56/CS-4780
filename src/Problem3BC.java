...
public class Problem3BC {
	 ...
	 public static void Problem3b(){
	        ...
	        String currstring;
	         ...
	         //Holding the message in this 2D array
	         
	        byte[][] newbytedoublearray = new byte[119][8];
	        int mycurrint;
	        
	         //Break message into 119 parts in this nested for loop
	         
	        for (int k = 0; k < 119; k++) {
	            int kint = k * 8;
	            currstring = message1.substring(kint, kint + 8);
	            String[] currstringarray = currstring.split("");
	            for (int j = 0; j < 8; j++) {
	                mycurrint = Integer.parseInt(currstringarray[j]);
	                newbytedoublearray[k][j] = (byte) mycurrint;
	            }
	        }
	        ...
	        byte[][] allkeys = new byte[1024][10];
	         ...
	         // Generate all 1024 possible keys, stored in allkeys[][]
	        ...
	        for (int i = 0; i < stringforwards.length; i++) {
	            for (int j = 0; j < stringbackwards.length; j++) {
	                ...
	                for (int k = 0; k < completearray.length; k++) {
	                    allkeys[line][k] = completearray[k];
	         ...
	         //Output decrypted message into a single line 952 bits. 1024 total decrypted messages. File is output.txt
	        ...
	        byte[] currentDecrypt;
	        byte[] messageout = new byte[952];
	        byte[][] messageoutfinal = new byte[1024][952];

	        for (int row = 0; row < 1024; row++) {
	            
	             //Setup current key from allkeys. Initialize currentKey to use in the next part.
	             
	            for (int col = 0; col < 10; col++) {
	                currentKey[col] = allkeys[row][col];
	                //System.out.print(allkeys[row][col] + " ");
	            }
	            for (int msgrow = 0; msgrow < 119; msgrow++) {
	                for (int msgcol = 0; msgcol < 8; msgcol++) {
	                    currentCiphertext[msgcol] = newbytedoublearray[msgrow][msgcol];
	                }
	                /*
	                 * Decryption here, just output what it gets onto file.
	                 */
	                currentDecrypt = Decrypt3b(currentKey, currentCiphertext);

	                
	                // Copy decrypt byte array to messageout array
	                 
	                System.arraycopy(currentDecrypt, 0, messageout, 8 * msgrow , 8);
	            }
	            
	           //  Push the 952 bits on to the messageoutfinal 2D array, each row a key
	             
	            for (int messcol = 0; messcol < 952; messcol++) {
	                messageoutfinal[row][messcol] = messageout[messcol];
	            }
	        }
	        
	         // Output messagefinal here.
	         
	        byte[] toconvert = new byte[952];
	        for (int row = 0; row < 1024; row++) {
	            for (int col = 0; col < 952; col++) {
	                toconvert[col] = messageoutfinal[row][col];
	            }
	            ...
	            if (ecount > 15){
	                ...
	                System.out.print("KEY: " + row  + " - " + setBoldText);
	                for (int i = 0; i < 10; i++) {
	                    System.out.print(allkeys[row][i] + " ");
	                    }
	 ...
	 public static void Problem3c(){
		 final long startTime = System.currentTimeMillis();
	        ...
	        for (int j = 0; j < parts; j++) {
	            ...
	            currentstring = message2.substring(jint, jint + 8);
    ...
    public static String message1 = "";
    public static String message2 = "";
    ...
    public static byte[] Decrypt3b(byte[] rawkey, byte[] ciphertext) {
        byte[] finplain;
    ...
    public static byte[] Decrypt3c(byte[] rawkey, byte[] ciphertext) {
        byte[] finplain;
        byte[] rightbyte = new byte[8];
        byte[] leftbyte = new byte[8];
        byte[] updatedIPop = new byte[8];
    ...
    public static byte[] TripleDecrypt(byte[] rawkey1, byte[] rawkey2, byte[] ciphertext) {
        byte[] result;
        result = Decrypt3c(rawkey1, Encrypt(rawkey2, Decrypt3c(rawkey1, ciphertext)));
        return result;
    }
